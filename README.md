...
完整记录见 [验收摘要](docs/artifact.json)、[本轮动作视觉检查](docs/qa/huntress-visual-qa.json) 和 [制作说明](docs/production.md)。
...
亲近动作采用轻歪头、眯眼微笑、恢复原姿势的五帧循环，保持脚底、卷轴和整体比例稳定。它占用现有的 `jumping` 槽位，保留 v2 格式。

| 行为 | 安装到 Codex | 本地预览的“互动体验” |
|---|---|---|
| 鼠标悬停身体 | 播放亲近槽位，是否循环由客户端控制 | 停留 350 ms 后回应一次 |
| 空闲时鼠标靠近 | 未在本项目中增加原生触发能力 | 180 px 内关注，210 px 外恢复待机 |
| 点击亲近 | 点击行为仍由客户端控制 | 播放一次，2.4 秒冷却，不排队重复 |
